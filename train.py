...
parser.add_argument('--expname', default='give_me_a_name_cifar', type=str,help='name of experiment')
...
parser.add_argument('--lossfact', default=2, type=float,help='loss factor')
...
parser.add_argument('--test', dest='test', action='store_true',help='To only run inference on test set')
parser.add_argument('--visdom', dest='visdom', action='store_true',help='Use visdom to track and plot')
parser.add_argument('--print-freq', '-p', default=25, type=int,help='print frequency (default: 10)')
...
def load_data():
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('/disk1/yaoyang/data', train=True, download=True,
                         transform=transform_train),
        batch_size=args.batch_size, shuffle=True, **kwargs)
    val_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('/disk1/yaoyang/data', train=False, transform=transform_test),
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #state, is_best, file_path='./results',filename='checkpoint.pth.tar'
        save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
            'best_prec1': best_prec1,
        }, is_best,file_path=args.save_path)
